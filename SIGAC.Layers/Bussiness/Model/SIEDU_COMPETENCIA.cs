//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace SIGAC.Layers.Bussiness.Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class SIEDU_COMPETENCIA
    {
        public short COMP_COMP { get; set; }
        public string COMP_DESCRI { get; set; }
        public short COMP_DOM_TPCOMPE { get; set; }
        public string COMP_USU_CREA { get; set; }
        public System.DateTime COMP_FECHA_CREA { get; set; }
        public string COMP_MAQUINA_CREA { get; set; }
        public string COMP_IP_CREA { get; set; }
        public string COMP_USU_MOD { get; set; }
        public Nullable<System.DateTime> COMP_FECHA_MOD { get; set; }
        public string COMP_MAQUINA_MOD { get; set; }
        public string COMP_IP_MOD { get; set; }
    }
}
